...
function resizeImage($inputFile, $newWidth, $outputFile, $checking=NULL)
{
...
function autoCropImage($inputFile, $outputFile)
{
	...
	saveImage($dst_r, $outputFile , 100, $extension);
	imagedestroy($dst_r);
}

function ImageBgColor($source_image, $destination, $tn_w, $tn_h, $quality = 100, $wmsource = false)
{
    ...
    $backgroundColor = imagecolorallocate($final, 238, 238, 238);
...
//				if(($sess_array['sess_access_group_type']==MEMBER_TYPE_STUDENT&&$sess_array['sess_student_allow_access_job']=='Y')||($sess_array['sess_access_group_type']==MEMBER_TYPE_ALUMNI&&$sess_array['sess_student_allow_access_job']=='Y')){				
	...
	function gen_url($_REQUEST1, $page_no){
		$i = 0;
		$str = "";
		foreach($_REQUEST1 as $key=>$value){
...
... but the principle is it ;) ... */ 
	...
	function split_words_chi($string, $nb_caracs, $separator){
	//$string = strip_tags(html_entity_decode($string));
    if( mb_strlen($string, "utf-8") <= $nb_caracs ){
        $final_string = $string;
    } else {
		$final_string = mb_substr($string,0,$nb_caracs,"UTF-8");
        $final_string .= $separator;
    }
    return $final_string;
}
	
	function split_words_eng($string, $nb_caracs, $separator){
    $string = strip_tags(html_entity_decode($string));
...
function autoBackgroundColor($source_image, $destination, $tn_w, $tn_h, $quality = 100, $wmsource = false)
{
    $info = getimagesize($source_image);
    $imgtype = image_type_to_mime_type($info[2]);
...
function customSelect($table,$custom_option, $select, $otherVal){	
	$custom_option_len = count($custom_option);
	if(isset($custom_option[0]['default_txt'])){
		$default_txt = $custom_option[0]['default_txt'];
	} else {
		$default_txt = $custom_option[1]['txt'];
	}
	
	$html ='<div class="customSelect"><a class="customSelectTrigger" href="#"><span>'.$default_txt.'</span></a>';
	$html .='<ul class="customSelectList">';
	for($i=1;$i<$custom_option_len;$i++){
		$customTxt = $custom_option[$i]['txt'];
		if(isset($custom_option[$i]['val'])){
			$customVal = $custom_option[$i]['val'];
		} else {
			$customVal = $customTxt;
		}
		
		$html .='<li data-val="'.$customVal.'"><span>'.$customTxt.'</span></li>';
	}
	$html .='</ul>';
	
	$html .='<select name="'.$table.'_'.$custom_option[0]['select_title'].'" class="customSelectListMob">';
	...
	for($i=1;$i<$custom_option_len;$i++){
		...
		$html .='<option value="'.$customVal.'" '.$selected.' >'.$customTxt.'</option>';
	}
	
	$html .='</select>';
	$html .='<input class="customSelectOther" name="'.$table.'_'.$custom_option[0]['select_title'].'_other" value="'.$otherVal.'" placeholder="(Please specify here)" disabled>';
	$html .='</div>';
	return $html;
}
